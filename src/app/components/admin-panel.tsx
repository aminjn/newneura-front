        ...
        <div className="absolute left-1/2 -translate-x-1/2 top-[36px]">
          <div ref={companyRef} className="relative flex items-center gap-1">
            <button onClick={() => setCompanyOpen(o => !o)}
              ...
              <span aria-label="Neura" style={{ display: 'inline-block', width: 15, height: 15, flexShrink: 0, background: 'var(--aw-eu-primary, #2E86FF)', WebkitMaskImage: `url(${logoMaskUrl()})`, maskImage: `url(${logoMaskUrl()})`, WebkitMaskRepeat: 'no-repeat', maskRepeat: 'no-repeat', WebkitMaskPosition: 'center', maskPosition: 'center', WebkitMaskSize: 'contain', maskSize: 'contain' }} />
...
function SingleAgentSettings({ agentId, onBack }: { agentId: string; onBack?: () => void }) {
  const { agents, setAgents, closeModal, showToast, openModal, defaultAgent, setDefaultAgent, assistantId, setAssistantId } = useApp();
  const a = agents.find(x => x.id === agentId);
  const [confirmDel, setConfirmDel] = useState(false);
  if (!a) return null;
  ...
  return (
    <div>
      ...
      <div className="flex gap-2">
        <button className="flex-1 py-2.5 px-5 border-none rounded-[10px] text-[13px] text-white cursor-pointer" style={{ background: 'var(--aw-primary, #2E86FF)', fontWeight: 600 }} onClick={() => { closeModal(); showToast('تنظیمات ' + a.name + ' ذخیره شد'); }}>ذخیره و بستن</button>
        {!confirmDel && (
...
export function ProfileContent() {
  const { role, agentTeam, userProfile, euProfile, setUserProfile, setEuProfile, closeModal, showToast, agents, customers } = useApp();
  ...
  const confirmHire = (m: typeof AGENT_MARKETPLACE[number]) => {
    ...
    setTimeout(() => {
      ...
      const newAgent: any = {
        ...
        init: m.name.charAt(0), locked: false, instructions: 'دستورالعمل پیش‌فرض عامل ' + m.name, accent: '#7B62FC',
...
export function CustomizeAgentContent({ agentId }: { agentId?: string } = {}) {
  ...
  return (
    <div>
      ...
      <button className="flex-1 py-2.5 rounded-[10px] border-none text-white cursor-pointer text-[13px]" style={{ background: 'var(--aw-primary, #2E86FF)', fontWeight: 600 }} onClick={() => {
        if (a) {
...
const TEAM_MARKET_ID: Record<string, string> = { assistant: 'm0', secretary: 'm1', marketing: 'm2', procurement: 'm6', sales: 'm7' };
...
function AdminAvatarBlock() {
  ...
  const openHire = () => {
    if (teamAgent) openModal('استخدام ایجنت', <HireAgentContent2 agentId={teamAgent.id} name={teamAgent.name} role={teamAgent.role} onDone={() => purchaseAgent(teamAgent.id, { name: teamAgent.name })} />, () => closeModal());
    else openModal('استخدام ایجنت', <HireAgentContent />, () => closeModal());
  };
  ...
  return (
    <div className="md:hidden flex flex-col items-center relative pb-3" style={{ zIndex: 30 }} data-chat-top-anchor>
      ...
      <EuAvatar
        ...
        name={displayName}
        nameOptions={group.length > 1 ? group.map(a => ({ id: a.id, name: a.name })) : undefined}
          ...
          <AnimatePresence mode="wait">
            {showBriefing ? (
              ...
              <>
                ...
                {adminScreen === 'inventoryScreen' && <motion.div key="inventory" className="flex flex-col h-full" variants={pageVariants} initial="initial" animate="animate" exit="exit"><InventoryScreen isProc={agentTeam === 'procurement'} /></motion.div>}
